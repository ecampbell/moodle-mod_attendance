 ...
 *
 */
class signinsheets_page_scanner {
     ...
     */
    public function __construct($att, $contextid, $maxquestions, $maxanswers) {
        if ($maxanswers > 26) {
    ...
    public function init_hotspots() {
        ...
        for ($x = 0; $x <= ($attendanceconfig->ID_digits - 1); $x++) {
            for ($y = 0; $y <= 9; $y++) {
    ...
    /**
     * Loads a temporary image file and creates and record for the table attendance_ss_scanned_pages if possible.
     *
    ...
    public function export_hotspots_userid($width) {
        ...
        for ($x = 0; $x < $attendanceconfig->ID_digits; $x++) {
            for ($y = 0; $y <= 9; $y++) {
    ...
    public function get_usernumber() {
         ...
         */
        if ($attendanceconfig->ID_digits > $this->id_digits) {
            // Fill the usernumber to the new length with 0 on the left!
            $usernumber = str_pad($usernumber, $attendanceconfig->ID_digits, "0", STR_PAD_LEFT);
        }
