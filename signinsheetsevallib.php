...
/**
 *
 * Functions for checking and evaluating scanned signin sheets of participants.
 *
 * @package       mod
...
function signinsheet_check_scanned_page($signinsheet, signinsheet_page_scanner $scanner, $scannedpage,
    ...
    if (empty($scannedpage->userkey)) {
        $scannedpage->userkey = $signinsheetconfig->ID_prefix . $usernumber . $signinsheetconfig->ID_postfix;
    }

    if ($scannedpage->status == 'ok' || $scannedpage->status == 'suspended') {
        if (!$user = $DB->get_record('user', array($signinsheetconfig->ID_field => $scannedpage->userkey))) {
            $scannedpage->status = 'error';
