...
if (!$scannedpage = $DB->get_record('attendance_ss_scanned_pages', array('id' => $pageid))) {
...
if (!$cm = get_coursemodule_from_instance('attendance', $attendance->id, $course->id)) {
    print_error('cmmissing', 'attendance', $CFG->wwwroot . '/course/view.php?id=' . $attendance->course, $attendance->id);
}
...
$scanner = new signinsheets_participants_scanner($offlinequiz, $context->id, 0, 0);
$corners = array();
...
if ($action == 'cancel') {
    ...
    echo "<center><input class=\"imagebutton\" type=\"submit\" value=\"" .
        get_string('signinsheetclosewindow', 'attendance')."\" name=\"submitbutton4\" onClick=\"self.close(); return false;\"></center>";
    echo '</html>';
    die;
...
onClick=\"self.close(); return false;\"><br />";
    ...
    if ($list = $DB->get_record('attendance_ss_p_lists', array('id' => $listid))) {
    ...
    // -------------------------------------------------------------
        ...
        $scannedpage->error = '';
        $listid = required_param('listid', PARAM_INT);
        $list = $DB->get_record('attendance_ss_p_lists', array('id' => $listid));
        $scannedpage->listnumber = intval($list->number);
...
$scannedpage = signinsheets_check_scanned_participants_page($offlinequiz, $scanner, $scannedpage, $USER->id, $coursecontext);
...
if ($scannedpage->status == 'ok') {
    $scannedpage = signinsheets_process_scanned_participants_page($offlinequiz, $scanner, $scannedpage, $USER->id, $coursecontext);
}
$listnumber = $scannedpage->listnumber;
...
if ($action == 'update') {
    ...
    if (!$insecuremarkings && !$unknownuser) {
        $scannedpage = signinsheets_submit_scanned_participants_page($offlinequiz, $scannedpage, $dbdata);
        echo '<html>';
        echo '<head><meta http-equiv="Content-Type" content="text/html; charset=utf-8" /></head><body>';
        if ($scannedpage->status == 'submitted') {
            echo get_string('pageimported', 'offlinequiz')."<br /><br />";
            $DB->set_field('attendance_ss_scanned_pages', 'error', '', array('id' => $pageid));
        } else {
...
echo html_writer::script('', $CFG->wwwroot.'/mod/attendance/lib/jquery/jquery-1.4.3.js');
echo html_writer::script('', $CFG->wwwroot.'/mod/attendance/lib/jquery/ui/jquery.ui.core.js');
echo html_writer::script('', $CFG->wwwroot.'/mod/attendance/lib/jquery/ui/jquery.ui.widget.js');
echo html_writer::script('', $CFG->wwwroot.'/mod/attendance/lib/jquery/ui/jquery.ui.mouse.js');
echo html_writer::script('', $CFG->wwwroot.'/mod/attendance/lib/jquery/ui/jquery.ui.draggable.js');
...
echo "<form method=\"post\" action=\"signinsheets_return.php\" id=\"cform\">\n";
...
echo "<img src=\"$CFG->wwwroot/mod/attendance/pix/spacer.gif\" border=\"0\" style=\"position:absolute; top:1260px; left:0px\">";

$fs = get_file_storage();
$imagefile = $fs->get_file($context->id, 'mod_attendance', 'imagefiles', 0, '/', $scannedpage->filename);

// Print image of the form sheet.
echo '<img name="formimage" src="' . $CFG->wwwroot . "/pluginfile.php/$context->id/mod_attendance/imagefiles/0/" .
...
if ($scannedpage->error == 'invalidlistnumber' && !$scanner->ontop && !$listchosen) {
    ...
    print_string('signinsheetlistnotdetected', 'attendance');
    echo "<br />";
    print_string('signinsheetselectlist', 'attendance');
    echo "<br />";
    echo "<select class=\"imagebutton\" name=\"listid\" onchange=\"set_list(this.value); true;\">\n";

    echo '<option value="0">'.get_string('choose').'...</option>';
    if ($lists = $DB->get_records('attendance_ss_p_lists', array('attendanceid' => $att->id), 'name ASC')) {
...
if ($sheetloaded) {
    foreach ($scanner->export_hotspots_participants(860) as $key => $hotspot) {
        ...
        if (isset($participants[$x])) {
            if ($participants[$x]->value == 'unknown') {
                echo "<img src=\"$CFG->wwwroot/mod/attendance/pix/blue.gif\" " .
                ...
                echo "<img src=\"$CFG->wwwroot/mod/attendance/pix/green.gif\" " .
                    " border=\"0\" id=\"p$x\" style=\"position:absolute; top:".
                    $hotspot->y."px; left:".$hotspot->x."px; cursor:pointer\" onClick=\"set_participant(this, $x)\">\n";
            } else {
                echo "<img src=\"$CFG->wwwroot/mod/attendance/pix/spacer.gif\" " .
                    " border=\"0\" id=\"p$x\" style=\"position:absolute; top:".
                    $hotspot->y."px; left:".$hotspot->x."px; cursor:pointer\" onClick=\"set_participant(this, $x)\">\n";
            }
        }
    ...
    if ($list) {
        $sql = "SELECT DISTINCT p.id, p.userid, u." . $attendanceconfig->ID_field . ", u.firstname, u.lastname
                  FROM {user} u,
...
lass', 'barcodeselect');}; checkinput(false);\">\n>";
            ...
            foreach ($users as $user) {
                ...
                echo ">" . substr($user->{$attendanceconfig->ID_field},
    ...
    echo "<form name=\"cornersform\" action=\"signinsheets_return.php\">\n";
